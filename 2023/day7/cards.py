...
def categorise_hand (hand_obj):
    # print (hand_obj["card_count"])
    pair_count = 0
    three_count = 0
    four_count = 0
    five_count = 0
    for hand in hand_obj["card_count"]:
        # print (hand, hand_obj["card_count"][hand])
    ...
    if pair_count == 1:
        if three_count == 1:
            ...
            hand_obj["score"] = (math.pow(15, 5) * 5) + score_hand(hand_obj["cards"])
        else:
            hand_obj["category"] = "One Pair"
            hand_obj["score"] = (math.pow(15, 5) * 2) + score_hand(hand_obj["cards"])
    elif pair_count == 2:
        hand_obj["category"] = "Two Pairs"
        # print (f'{hand_obj["cards"]}, ')
        hand_obj["score"] = (math.pow(15, 5) * 3) + score_hand(hand_obj["cards"])
    elif three_count == 1:
        hand_obj["category"] = "Three of a Kind"
        hand_obj["score"] = (math.pow(15, 5) * 4)+ score_hand(hand_obj["cards"])
    elif four_count == 1:
        hand_obj["category"] = "Four of a Kind"
        hand_obj["score"] = (math.pow(15, 5) * 6) + score_hand(hand_obj["cards"])
    elif five_count == 1:
        hand_obj["category"] = "Five of a Kind"
        hand_obj["score"] = (math.pow(15, 5) * 7) + score_hand(hand_obj["cards"])
    else:
        hand_obj["category"] = "High Card"
        hand_obj["score"] = math.pow(15, 5) + score_hand(hand_obj["cards"])
...
for hand_details in hands:
    ...
    card_count = Counter(hand_details[0])
    hand_obj = {
        "raw" : hand_details,
        "cards" : hand_details[0],
...
for i in range(0, len(sorted_hand_objs)):
    print (sorted_hand_objs[i]["category"], sorted_hand_objs[i]["score"], sorted_hand_objs[i]["cards"], (i+1) * sorted_hand_objs[i]["bid"])
